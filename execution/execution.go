...
func New(
	...
	return newOperator(expr, selectorPool, opts, hints, stats)
}

func newOperator(expr parser.Expr, storage *engstore.SelectorPool, opts *query.Options, hints storage.SelectHints, stats *stats.Statistics) (model.VectorOperator, error) {
	switch e := expr.(type) {
	...
	case *parser.VectorSelector:
		...
		return newShardedVectorSelector(filter, opts, stats, e.Offset)
	...
	case *logicalplan.FilteredSelector:
		...
		return newShardedVectorSelector(selector, opts, stats, e.Offset)
	...
	case *parser.Call:
		...
		if e.Func.Name == "histogram_quantile" {
			...
			for i := range e.Args {
				next, err := newOperator(e.Args[i], storage, opts, hints, stats)
				if err != nil {
					return nil, err
				}
			...
			case *parser.MatrixSelector:
				...
				for i := 0; i < numShards; i++ {
					operator := exchange.NewConcurrent(
						scan.NewMatrixSelector(model.NewVectorPool(stepsBatch), filter, call, e, opts, stats, t.Range, vs.Offset, i, numShards),
						2,
					)
	...
	case *parser.AggregateExpr:
		...
		next, err := newOperator(e.Expr, storage, opts, hints, stats)
		if err != nil {
			return nil, err
		}

		if e.Param != nil {
			paramOp, err = newOperator(e.Param, storage, opts, hints, stats)
			if err != nil {
				return nil, err
			}
		}
	...
	case *parser.BinaryExpr:
		if e.LHS.Type() == parser.ValueTypeScalar || e.RHS.Type() == parser.ValueTypeScalar {
			return newScalarBinaryOperator(e, storage, opts, hints, stats)
		}

		return newVectorBinaryOperator(e, storage, opts, hints, stats)

	case *parser.ParenExpr:
		return newOperator(e.Expr, storage, opts, hints, stats)
	...
	case *parser.StepInvariantExpr:
		...
		next, err := newOperator(e.Expr, storage, opts.WithEndTime(opts.Start), hints, stats)
		if err != nil {
			return nil, err
		}
	...
	case logicalplan.Deduplicate:
		...
		for i, expr := range e.Expressions {
			operator, err := newOperator(expr, storage, opts, hints, stats)
			if err != nil {
				return nil, err
			}
	...
	case logicalplan.RemoteExecution:
		...
		return exchange.NewConcurrent(remote.NewExecution(qry, model.NewVectorPool(stepsBatch), opts, stats), 2), nil
...
func newShardedVectorSelector(selector engstore.SeriesSelector, opts *query.Options, stats *stats.Statistics, offset time.Duration) (model.VectorOperator, error) {
	numShards := runtime.GOMAXPROCS(0) / 2
	if numShards < 1 {
		numShards = 1
	}
	...
	for i := 0; i < numShards; i++ {
		operator := exchange.NewConcurrent(
			scan.NewVectorSelector(
				model.NewVectorPool(stepsBatch), selector, opts, stats, offset, i, numShards), 2)
		operators = append(operators, operator)
	}
...
func newVectorBinaryOperator(e *parser.BinaryExpr, selectorPool *engstore.SelectorPool, opts *query.Options, hints storage.SelectHints, stats *stats.Statistics) (model.VectorOperator, error) {
	leftOperator, err := newOperator(e.LHS, selectorPool, opts, hints, stats)
	if err != nil {
		return nil, err
	}
	rightOperator, err := newOperator(e.RHS, selectorPool, opts, hints, stats)
	if err != nil {
		return nil, err
	}
...
func newScalarBinaryOperator(e *parser.BinaryExpr, selectorPool *engstore.SelectorPool, opts *query.Options, hints storage.SelectHints, stats *stats.Statistics) (model.VectorOperator, error) {
	lhs, err := newOperator(e.LHS, selectorPool, opts, hints, stats)
	if err != nil {
		return nil, err
	}
	rhs, err := newOperator(e.RHS, selectorPool, opts, hints, stats)
	if err != nil {
		return nil, err
	}
