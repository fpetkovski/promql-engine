...
func makeAccumulatorFunc(expr parser.ItemType, param parser.Expr) (newAccumulatorFunc, error) {
	...
	case "quantile":
		...
		if !ok {
			return nil, fmt.Errorf("invalid type for quantile argument, got %s, want %s", parser.ValueTypeScalar, q.Type())
		}
